...
func find_cheapest_path(graph []*vertex, start point, finish point) int {
	...
	return traverse(h, graph, visited, finish, make(map[point]struct{}), math.MaxInt)
}

func traverse(h *heap.Heap[move], graph []*vertex, visited map[vertexWithDir]int, finish point, all_shortest_path_vertexes map[point]struct{}, shortest_path_len int) int {
	current_move, err := h.TakeTop()
	if err != nil {
		// this line is called if last path we traverse leads to nowhere
		return len(all_shortest_path_vertexes) + 1 // +1 for finish
	}
	...
	// if this is finish and still shortest path - save all vertexes and continue
	// if this is finish and not shortest path - time to go away
	if current_move.Move.vertex.p == finish {
		if current_move.Move.accumulated_cost <= shortest_path_len {
			for _, x := range current_move.Move.accumulated_path {
				all_shortest_path_vertexes[x.vertex.p] = struct{}{}
			}

			return traverse(h, graph, visited, finish, all_shortest_path_vertexes, current_move.Move.accumulated_cost)
		} else {
			// this line is called if last path we traverse still leads to finish
			return len(all_shortest_path_vertexes) + 1 // +1 for finish
		}
	}
	...
	return traverse(h, graph, visited, finish, all_shortest_path_vertexes, shortest_path_len)
}
