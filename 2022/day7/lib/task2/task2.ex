defmodule Task2 do
  ...
  def solution(contents) do
    ...
    dirs_above(sized, need_to_free(sized.size))
    |> smallest()
  end

  @spec need_to_free(Integer.t()) :: Integer.t()
  defp need_to_free(size) do
    30_000_000 - (70_000_000 - size)
  end
  ...
  @spec dirs_above(%Fs.Dir{}, Integer.t()) :: [Fs.Dir]
  defp dirs_above(root, limit) do
    res =
      Enum.reduce(root.children, [], fn x, acc ->
        node =
          case x do
            dir when is_struct(x, Fs.Dir) ->
              dirs_above(dir, limit)
  ...
  @spec smallest([Fs.Dir]) :: integer()
  defp smallest(dirs) do
    dir = Enum.sort_by(dirs, fn x -> x.size end, :asc) |> Enum.at(0)
